/* eslint-env jest */
import { filterContextPath, toAnalyticsPath } from '../index';

describe('utils', () => {
	test('filterContextPath', () => {
		const id = 'c';
		const context1 = filterContextPath(
			[{ ntiid: 'a' }, { ntiid: 'b' }, { ntiid: id }],
			id
		);
		...
		const context3 = filterContextPath(
			[{}, { ntiid: 'b' }, { ntiid: id }],
			id
		);
		expect(context3).toEqual([{ ntiid: 'b' }]);

		const context4 = filterContextPath(
			[{}, { ntiid: 'b' }, { ntiid: id }],
			id
		);
